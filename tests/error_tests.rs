...
fn test_error_response_type_display() {
    assert_eq!(
        ErrorResponseType::InvalidRequest.to_string(),
        "invalid_request"
    );
    assert_eq!(
        ErrorResponseType::InvalidClient.to_string(),
        "invalid_client"
    );
    assert_eq!(ErrorResponseType::InvalidGrant.to_string(), "invalid_grant");
    assert_eq!(
        ErrorResponseType::UnauthorizedClient.to_string(),
        "unauthorized_client"
    );
    assert_eq!(
        ErrorResponseType::UnsupportedGrantType.to_string(),
...
fn test_apple_error_display_variants() {
    assert_eq!(
        AppleError::IoError("disk".into()).to_string(),
        "IO error: disk"
    );
    assert_eq!(
        AppleError::Base64Error("bad".into()).to_string(),
        "Base64 error: bad"
    );
    assert_eq!(
        AppleError::PemError("pem".into()).to_string(),
        "PEM error: pem"
    );
    assert_eq!(
        AppleError::KeyParseError("key".into()).to_string(),
        "Key parse error: key"
    );
    assert_eq!(
        AppleError::JwtError("jwt".into()).to_string(),
        "JWT error: jwt"
    );
    assert_eq!(
        AppleError::HttpError("http".into()).to_string(),
        "HTTP error: http"
    );
    assert_eq!(
        AppleError::JsonError("json".into()).to_string(),
        "JSON error: json"
    );
    assert_eq!(
        AppleError::TimeError("time".into()).to_string(),
        "Time error: time"
    );
    assert_eq!(
        AppleError::UnrecognizedError("unk".into()).to_string(),
        "Unrecognized error: unk"
    );
}
...
fn test_error_constants() {
    ...
    match &ERROR_RESPONSE_UNAUTHORIZED_CLIENT {
        AppleError::ResponseError(e) => {
            assert_eq!(e.error_type, ErrorResponseType::UnauthorizedClient)
        }
        _ => panic!("Expected ResponseError"),
    }
    ...
    match &ERROR_RESPONSE_UNSUPPORTED_GRANT_TYPE {
        AppleError::ResponseError(e) => {
            assert_eq!(e.error_type, ErrorResponseType::UnsupportedGrantType)
        }
        _ => panic!("Expected ResponseError"),
    }
...
mod cloudkit_error_tests {
    ...
    fn test_cloudkit_error_code_parse_known() {
        assert_eq!(
            CloudKitErrorCode::parse("BAD_REQUEST"),
            CloudKitErrorCode::BadRequest
        );
        assert_eq!(
            CloudKitErrorCode::parse("AUTHENTICATION_FAILED"),
            CloudKitErrorCode::AuthenticationFailed
        );
        assert_eq!(
            CloudKitErrorCode::parse("ACCESS_DENIED"),
            CloudKitErrorCode::AccessDenied
        );
        assert_eq!(
            CloudKitErrorCode::parse("NOT_FOUND"),
            CloudKitErrorCode::NotFound
        );
        assert_eq!(
            CloudKitErrorCode::parse("ZONE_NOT_FOUND"),
            CloudKitErrorCode::ZoneNotFound
        );
        assert_eq!(
            CloudKitErrorCode::parse("CONFLICT"),
            CloudKitErrorCode::Conflict
        );
        assert_eq!(
            CloudKitErrorCode::parse("EXISTS"),
            CloudKitErrorCode::Exists
        );
        assert_eq!(
            CloudKitErrorCode::parse("QUOTA_EXCEEDED"),
            CloudKitErrorCode::QuotaExceeded
        );
        assert_eq!(
            CloudKitErrorCode::parse("AUTHENTICATION_REQUIRED"),
            CloudKitErrorCode::AuthenticationRequired
        );
        assert_eq!(
            CloudKitErrorCode::parse("THROTTLED"),
            CloudKitErrorCode::Throttled
        );
        assert_eq!(
            CloudKitErrorCode::parse("INTERNAL_ERROR"),
            CloudKitErrorCode::InternalError
        );
        assert_eq!(
            CloudKitErrorCode::parse("TRY_AGAIN_LATER"),
            CloudKitErrorCode::TryAgainLater
        );
    }
